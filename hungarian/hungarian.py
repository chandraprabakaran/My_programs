...
def jkrsa(p, q, plain_text=""):
    ...
    privateKey = (k,d,n)
    if plain_text == "":
        plain_message = reading_file('hungarian_data.csv')
    else:
    ...
    import numpy as np
    two_split = np.array_split(((ds.iloc[0]).values), 2940)
    df = pd.DataFrame(two_split)
    df.to_csv("sample1.csv")
    datas = pd.read_csv("sample1.csv")
    datas.columns = ["index","data"]
    split_data = datas["data"].str.split(",")
    new = split_data.to_list()
    new_df = pd.DataFrame(new)
    # print(new_df)
    new_df.to_csv("hungarian_data1.csv",header=False,index=False)
...
def main():
    ...
    flag = input("Enter:-\n\t- 'f' to read from file => hungarian_data.csv.\n\t- 'm' to enter plaintext message.\n")
    if flag == 'f':
        return jkrsa(p,q)
    elif flag == 'm':
